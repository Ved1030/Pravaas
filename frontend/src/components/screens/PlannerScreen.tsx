  ...
  return (
    <>
          ...
          <div className="space-y-6">
            ...
            <motion.div variants={fadeUp} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              >
                ...
                <span className={`text-sm font-bold ${priorityMeta.color}`}>{priorityMeta.label}</span>
              </motion.div>
          ...
          <div className="space-y-5">
            ...
            <motion.div variants={fadeUp} className="flex items-center justify-between">
              ...
              <button className="text-sm font-semibold text-[#3c7689] hover:underline flex items-center gap-1">
                Sort: Fastest <ChevronDown size={14} />
              </button>
            </motion.div>
            ...
            {(routeResults || []).map((route) => {
              ...
              return (
                <motion.div
                  key={route.id}
                  variants={fadeUp}
                  className={`bg-white rounded-2xl shadow-sm overflow-hidden transition-all ${isRecommended
                    ? 'border-2 border-[#1b3a2a]/30 ring-1 ring-[#1b3a2a]/10'
                  ...
                  <div className="p-6">
                      ...
                      <div className="text-right">
                        <div className="flex flex-col items-end">
                          <div className="flex items-baseline gap-1">
                            <span className="text-3xl font-bold text-gray-900 tracking-tight tabular-nums">{route.totalTime}</span>
                            <span className="text-sm font-medium text-gray-400">min</span>
                          </div>
                          {(timeMode === 'depart' || timeMode === 'arrive') && selectedTime && (
                            <div className="text-xs font-bold text-[#1b3a2a] bg-[#1b3a2a]/10 px-2 py-0.5 rounded mb-1">
                              {timeMode === 'depart' ? 'Departs ' : 'Arrives '}{selectedTime}
                            </div>
                          )}
                        </div>
                        <span className="text-sm font-semibold text-gray-500">₹{route.cost}</span>
                      </div>
                    </div>
                    ...
                    {isRecommended && explanation && (
                      ...
                      >
                        <div className="flex items-start gap-2">
                          ...
                          <p className="text-xs font-medium text-[#1b3a2a] leading-relaxed">{explanation}</p>
                        </div>
                      </motion.div>
                    )}
                    ...
                    {isRecommended && liveRoutes && route.insights && (
                      <div className="flex flex-wrap gap-2 mt-2 mb-3">
                        {route.insights.timeSaved > 0 && (
                          <span className="text-[10px] font-bold bg-blue-50 text-blue-700 px-2 py-1 rounded-lg">⚡ Saves {route.insights.timeSaved} min</span>
                        )}
                        {route.insights.avoidedTraffic && (
                          <span className="text-[10px] font-bold bg-amber-50 text-amber-700 px-2 py-1 rounded-lg">🚦 Avoids heavy traffic</span>
                        )}
                        {route.insights.costSaved > 0 && (
                          <span className="text-[10px] font-bold bg-emerald-50 text-emerald-700 px-2 py-1 rounded-lg">₹{route.insights.costSaved} cheaper</span>
                        )}
                        {route.insights.predictedDelay > 0 && (
                          <span className="text-[10px] font-bold bg-purple-50 text-purple-700 px-2 py-1 rounded-lg">+{route.insights.predictedDelay} min delay avoided</span>
                        )}
                      </div>
                    )}
                    ...
                    {isRecommended && savedTime && savedTime > 0 && (
                      <div className="mb-4 bg-blue-50 border border-blue-200 rounded-xl px-3 py-2 flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-blue-700">Saves {savedTime} min vs slowest route</span>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-2 mt-4 flex-wrap">
                      <div className="bg-gray-50 border border-gray-200 rounded-lg px-2.5 py-1.5 flex items-center gap-1.5">
                        ...
                        <span className="text-[11px] font-bold text-gray-600">{legs} Legs</span>
                      </div>
                      <div className={`${traffic.bg} border ${route.trafficLevel === 'high' ? 'border-red-200' : route.trafficLevel === 'low' ? 'border-emerald-200' : 'border-amber-200'
                        ...
                        <span className={`text-[11px] font-bold ${traffic.color}`}>{traffic.label} Traffic</span>
                      </div>
                    ...
                    <div className="flex gap-3 mt-5">
                      ...
                      <motion.button
                        ...
                        className={`flex-1 py-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition-all duration-300 ${isExpanded
                      ...
                      >
                        {isExpanded
                          ? <><ChevronDown size={18} className="rotate-180" /> Collapse</>
                          : <><ChevronRight size={18} /> Select This Route</>
                        }
                      </motion.button>
                      ...
                      <motion.button
                        ...
                        onClick={() => handleScheduleRoute({
                          ...route,
                          from: route.from || origin,
                          to: route.to || dest,
                        })}
                        title="Schedule this route for later"
                        className="py-3 px-4 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition-all duration-200 bg-white border-2 border-[#1b3a2a]/20 text-[#1b3a2a] hover:bg-[#1b3a2a]/5 hover:border-[#1b3a2a]/40 shadow-sm"
                      >
                                ...
                                >
                                  ...
                                  <div className="pb-4 last:pb-0 flex-1 min-w-0">
                                    <div className="flex items-center justify-between">
                                      <p className="font-semibold text-sm text-gray-900">{seg.label}</p>
                      ...
                      {(routeResults || []).map((r, i) => {
                        ...
                        return (
                          <tr key={r.id} className={`${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} hover:bg-blue-50/50 transition-colors`}>
                            ...
                            <td className="px-4 py-2.5 font-semibold text-gray-900 border-b border-gray-100 tabular-nums">{r.totalTime}m</td>
                            <td className="px-4 py-2.5 font-semibold text-gray-900 border-b border-gray-100 tabular-nums">₹{r.cost}</td>
      ...
      <ScheduleModal
        ...
        route={routeToSchedule ? {
          id: String(routeToSchedule.id),
          from: routeToSchedule.from || origin,
          to: routeToSchedule.to || dest,
          badge: routeToSchedule.badge,
          badgeColor: routeToSchedule.badgeColor,
          totalTime: routeToSchedule.totalTime,
          cost: routeToSchedule.cost,
          routeType: (routeToSchedule as any).routeType,
          confidence: routeToSchedule.confidence,
          segments: routeToSchedule.segments,
          scheduleDate: (routeToSchedule as any).scheduleDate,
          scheduleTime: (routeToSchedule as any).scheduleTime,
        } : null}
      />
    </>
  );
